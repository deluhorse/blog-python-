            ...
            if not (file_name.endswith('.pyc') or file_name.startswith('__') or file_name.startswith('base')):
                try:
                    ...
                    if hasattr(controller_module, 'Controller'):
                        ...
                        route.append((r"" + route_path + controller_key.replace(root_path + '.', '').
                                      replace('.', '/').replace('/controller', '') + '/*', controller_item))

                        print route_path + controller_key.replace(root_path + '.', '').replace('.', '/').replace('/controller', '') + '/*'
