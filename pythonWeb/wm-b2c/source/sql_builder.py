...
class SqlBuilder(object):
    ...
    def build_paginate(self, str_table_name, dic_data):
        ...
        # 页码
        int_page = dic_data[SqlConstants.PAGE] if SqlConstants.PAGE in dic_data else 1
        int_page_num = dic_data[SqlConstants.LIMIT][1] if SqlConstants.LIMIT in dic_data else '10'
        int_start_limit = (int_page - 1) * int(int_page_num)

        dic_data[SqlConstants.LIMIT] = [str(int_start_limit), int_page_num]
    ...
    def build_insert(self, str_table_name, dic_data):
        ...
        dic_data = self.format_data(dic_data)
        str_table_name = self.build_table_name(str_table_name)
        return "insert into %s (%s) values (%s)" % (str_table_name, dic_data[SqlConstants.KEY],
    ...
    def build_batch_insert(self, str_table_name, dic_data):
        ...
        return "insert into %s (%s) values %s" % (str_table_name, dic_data[SqlConstants.KEY],
                                                  ','.join(dic_data[SqlConstants.BATCH_VAL]))
    ...
    def format_data(self, dic_data):
        ...
        # key
        dic_data[SqlConstants.KEY] = dic_data[SqlConstants.KEY] if SqlConstants.KEY in dic_data else ''

        # val
        dic_data[SqlConstants.VAL] = dic_data[SqlConstants.VAL] if SqlConstants.VAL in dic_data else ''
    ...
    def get_orm_condition(self, fields, params):
        """
        构造orm条件语句
        :param fields: 
        :return: 
        ...
        value_list = []
        for field in fields:
            if field['key'] in params and params[field['key']]:
                condition += ' and %s' % field['condition']
                value_list.append(params[field['key']])
        return condition, value_list
